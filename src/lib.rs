...
impl<M: AddressManager> ServiceProtocol for DiscoveryProtocol<M> {
    ...
    fn connected(&mut self, context: ProtocolContextMutRef, _version: &str) {
        ...
        self.sessions.insert(
            session.id,
            SessionState::new(context, self.peer_key.clone()),
        );
    }
                    ...
                    DiscoveryMessage::GetNodes {
                        ...
                        if let Some(state) = self.sessions.get_mut(&session.id) {
                            ...
                            if let Some(port) = listen_port {
                                ...
                                let mut remote_addr = state.remote_addr.to_inner().clone();
                                if let Some(peer_key) = peer_key.clone() {
                                    trace!("received peer key: {:?}", peer_key);
                                    remote_addr.push(Protocol::Tls(Cow::Borrowed(&peer_key)));
                                    state.remote_addr.update_inner(remote_addr.clone());
                                }
                                state.addr_known.insert(remote_addr.clone());
                                // add client listen address to manager
                                if let RemoteAddress::Listen(_) = state.remote_addr {
                                    self.addr_mgr.add_new_addr(session.id, remote_addr);
                                }
                            }
